...
if ($_FILES["logo"]["error"] !=0) {// super global $_FILES é responsável por recuperar arquivos, é a única super global que é uma matriz
   die("não foi possével fazer upload, erro: ".$_UP["erros"][$_FILES["logo"]["error"]]);
   exit;  // para encerrar o código
}

$ext = explode('.', $_FILES["logo"]["name"]);// explode é uma função que divide uma string e divide entre várias strings, primeiro coloca- se o caractere delimitador
...
if (move_uploaded_file($_FILES["logo"]["tmp_name"], $_UP["pasta"].$nome_final)) {//dentro de um if(se), move_upload precisa de dois parâmetros -> o lugar onde esta(local temporário) e o lugar para onde irá(local de destino)
   ...
   //comando insert
   $sql = "INSERT INTO fornecedor VALUES(0,'$nome','$cnpj','$logo')";
   //executnado o comando insert
   $sqlInsert = mysqli_query($conexao,$sql);
?>
   <script type="text/javascript">
      alert("Produto cadastrado com sucesso");  
   </script>
   <meta  http-equiv="refresh" content="0;url=fornecedor.php">
<?php

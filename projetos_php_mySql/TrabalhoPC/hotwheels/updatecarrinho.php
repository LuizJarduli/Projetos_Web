...
if (move_uploaded_file($_FILES["img"]["tmp_name"], $_UP["pasta"].$nome_final)) {
		...
		$sql = "UPDATE carrinho SET nome=:pnome,imgcarrinho=:pimgcarrinho,preco=:preco,qtdcarrinho=:qtddisponivel WHERE idcarrinho=:pidcarrinho";

		$sqlInsert = $con->prepare($sql);
		$sqlInsert->bindParam(":pnome",$nome);
		$sqlInsert->bindParam(":pimgcarrinho",$imgcarrinho);
		$sqlInsert->bindParam(":preco",$preco);
		$sqlInsert->bindParam(":qtddisponivel",$qtd);
		$sqlInsert->bindParam(":pidcarrinho",$id);
